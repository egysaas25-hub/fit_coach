...
import { Phone, MessageSquare, AlertCircle, Loader2, HelpCircle } from 'lucide-react';
...
export default function UnifiedLogin() {
  ...
  const handleVerifyOTP = async (otpCode: string) => {
    ...
    try {
      ...
      // If multi-tenant, show tenant selector
      if (tenants && tenants.length > 1) {
        setStep('tenant');
      } else {
        // For single tenant, set default and go to role selection
        if (tenants && tenants.length === 1) {
      ...
      (window as any).dataLayer = (window as any).dataLayer || [];
      (window as any).dataLayer.push({ event: 'EVT-AUTH-FAIL', phone: fullPhone });
    }
  };
        ...
        <CardHeader className="space-y-1">
          <div className="flex items-center gap-2 mb-4 justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold">{settings?.branding?.title || settings?.general?.appName || 'FitCoach'}</span>
            </div>
            <div className="w-28">
          ...
          <CardDescription>
            ...
            {step === 'tenant' && 'Select your workspace'}
            {step === 'role' && 'Choose how you want to access FitCoach'}
          </CardDescription>
