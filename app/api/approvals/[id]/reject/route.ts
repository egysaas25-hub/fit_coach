...
/**
 * POST /api/approvals/[id]/reject - Reject an approval workflow
 * Body:
 * - reviewed_by: ID of the reviewer (team member)
 * - notes: Rejection reason/notes (required for rejection)
 */
export async function POST(
  ...
  try {
    ...
    // Update approval workflow to rejected
    ...
    if (existingApproval.entity_type === 'exercise') {
      // Update exercise status to rejected
      ...
      // Update nutrition plan status to rejected and keep it inactive
      ...
      // Update workout status to rejected (if workout table has status field)
    ...
    return NextResponse.json({
      ...
      message: "Approval workflow rejected successfully",
    })
  } catch (error) {
